...
import { LayoutDashboard, ArrowLeftRight, BookOpen, Table2, ListTree, TrendingUp, BarChart3, Upload, Building2, ChevronDown } from 'lucide-react'
...
const NAV = [
  ...
  { label: 'Gráficos',        href: '/reportes?tab=graficos', icon: BarChart3 },
]
...
export default function Sidebar({ pendingCount = 0 }: { pendingCount?: number }) {
  ...
  return (
    <aside className="w-52 min-w-[208px] flex flex-col" style={{ background: '#042C53' }}>
      ...
      <button className="mx-3 mt-3 flex items-center gap-2 px-3 py-2 rounded-lg bg-white/8 border border-white/12 hover:bg-white/12 transition-colors">
        ...
        <div className="text-left min-w-0">
          ...
          <div className="text-xs text-white/35 leading-tight">Corp · cambiar</div>
        </div>
      ...
      <nav className="flex-1 px-2 py-3 overflow-y-auto">
        {NAV.map((item, i) => {
          if ('section' in item) return (
            <div key={i} className="text-xs text-white/30 font-medium px-3 mt-3 mb-1 tracking-wide uppercase" style={{ fontSize: 9 }}>{item.section}</div>
          )
          const Icon = item.icon!
          const active = pathname === item.href || (item.href !== '/' && pathname.startsWith(item.href.split('?')[0]))
          return (
            <div key={i} className={clsx('nav-item', active && 'active')} onClick={() => router.push(item.href)}>
              <Icon size={14} />
      ...
      <div className="px-3 py-3 border-t border-white/10">
        <button onClick={() => router.push('/upload')} className="nav-item w-full">
          <Upload size={14} />
